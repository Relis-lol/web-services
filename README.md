...
`250 Roger, accepting` quittiert — geprüft wird erst beim Versand anhand des
`From:`-Headers. Eine nicht freigegebene Adresse führt deshalb nicht zu einem
Verbindungsfehler, sondern zu einer abgelehnten oder als Spam eingestuften
Mail. Eine Probe auf Verbindungsebene ist also **kein** Nachweis.

Solange `SMTP_FROM` leer ist, meldet das Formular ehrlich einen Fehler und
täuscht keinen Versand vor.

### E-Mail-Adressen und Cloudflare

Auf der Zone ist **Email Address Obfuscation** aktiv. Cloudflare ersetzt
`mailto:`-Links im HTML durch `[email protected]` und stellt die Adresse erst
per JavaScript wieder her — bei einer Pflichtangabe nach § 5 DDG ungeeignet.

Gelöst **ohne Änderung an der Cloudflare-Zone**: Die betroffenen Stellen in
`impressum.html` und `datenschutz.html` stehen in
`<!--email_off--> … <!--/email_off-->`. Diese Blöcke lässt Cloudflare
unangetastet. Nachgemessen im ausgelieferten HTML: beide Adressen im
Klartext, `mailto:`-Links intakt, null Verschleierung.

Beim Ergänzen weiterer Adressen: **neue `mailto:`-Links ebenfalls in
`email_off` einfassen**, sonst greift die Verschleierung dort wieder.

Auf der Startseite entsteht der Kontaktbereich per JavaScript — den sieht
Cloudflares HTML-Umschreibung ohnehin nicht.
